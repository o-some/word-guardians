(()=>{
  ...
  function ensureUi(){
    ...
    let slot=$('emergencyHeaderSlot');
    if(!slot){
      slot=document.createElement('div');
      slot.id='emergencyHeaderSlot';
      slot.className='emergencyHeaderSlot';
      top.insertBefore(slot,hud);
    }

    if(ability.mounted){
      const wrap=document.querySelector('.emergencyWrap');
      if(wrap&&wrap.parentElement!==slot)slot.appendChild(wrap);
      return;
    }

    const old=document.querySelector('.emergencyWrap');
    if(old)old.remove();

    const wrap=document.createElement('div');
    wrap.className='emergencyWrap emergencyHeader';
    ...
    if(guide&&!guide.querySelector('[data-emergency-info]')){
      ...
      item.dataset.emergencyInfo='1';
      item.innerHTML='🚨 <b>Insel-Notruf</b> – der rote Alarmknopf startet READY. Er besiegt alle aktuell sichtbaren Gegner unabhängig von ihren HP. Danach lädt er 3:00 Minuten nach; die rote Farbe kehrt während des Cooldowns sichtbar zurück.';
  ...
  function updateUi(){
    ...
    btn.style.setProperty('--abilityGray',(ready?0:Math.max(0,.94-progress*.94)).toFixed(3));
    btn.style.setProperty('--abilitySat',(ready?1.12:.18+progress*.94).toFixed(3));
  ...
  function fire(){
    ...
    if(!targets.length){
      const fb=$('fb');if(fb)fb.textContent='🚨 Insel-Notruf ist READY – aber gerade sind keine Piraten auf dem Feld.';
      const btn=$('emergencyBtn');if(btn){btn.classList.add('noTarget');setTimeout(()=>btn.classList.remove('noTarget'),420)}
      return;
    }
    ability.firing=true;
  ...
  function loop(ts){
    const dt=Math.min(.1,Math.max(0,(ts-ability.last)/1000));
    ability.last=ts;
    ensureUi();
