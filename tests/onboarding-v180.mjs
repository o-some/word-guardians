...
async function run(name,browserType,viewport){
  ...
  if(!version.startsWith('v1.9.2'))throw new Error(`${name}: expected visible v1.9.2, got ${version}`);
  ...
  await browser.close();
  console.log(`PASS ${name} · v1.9.2 onboarding + readable guide + 60s Notruf`);
}
...
console.log('ONBOARDING_V192_PASS');
